...
# Load pre-trained language classifier
try:
    language_tokenizer = DistilBertTokenizer.from_pretrained("distilbert-base-multilingual-cased")
    language_classifier = DistilBertForSequenceClassification.from_pretrained("language_classifier")
    language_label_encoder = joblib.load("language_label_encoder.joblib")
    logger.info("Loaded existing language classifier")
except Exception as e:
    logger.error(f"Failed to load language classifier: {e}")
    language_classifier, label_encoder = None, None
...
def detect_language(message: str, specified_language: str) -> str:
    ...
    try:
        ...
        if processed_input in embedding_cache:
            ...
            encodings = language_tokenizer(processed_input, truncation=True, padding=True, max_length=32, return_tensors="pt")
            with torch.no_grad():
                logits = language_classifier(**encodings).logits
            embedding_cache[processed_input] = logits
        probs = torch.softmax(logits, dim=-1).numpy()[0]
        predicted_idx = probs.argmax()
        predicted_lang = language_label_encoder.inverse_transform([predicted_idx])[0]
        confidence = probs[predicted_idx]
        probs_dict = dict(zip(language_label_encoder.classes_, probs))
...
try:
    intent_tokenizer = DistilBertTokenizer.from_pretrained("intent_classifier")
    intent_classifier = DistilBertForSequenceClassification.from_pretrained("intent_classifier")
    intent_label_encoder = joblib.load("intent_label_encoder.joblib")
    logger.info("Loaded existing intent classifier")
except Exception as e:
    logger.error(f"Failed to load intent classifier: {e}")
    intent_tokenizer, intent_classifier, intent_label_encoder = None, None, None

def detect_intent(message: str) -> str:
    try:
        processed_text = preprocess_text(message)
        encodings = intent_tokenizer(processed_text, truncation=True, padding=True, max_length=32, return_tensors="pt")
        with torch.no_grad():
            outputs = intent_classifier(**encodings)
        predicted_label = torch.argmax(outputs.logits, dim=1).item()
        intent = intent_label_encoder.inverse_transform([predicted_label])[0]
...
# Update the filter_context_by_language function to include source_entry matching
def filter_context_by_language(context: list, language: str) -> str:
    ...
    for doc in context:
        if doc.metadata.get("language") == language:
            ...
            # Check for source_entry and fetch corresponding fiscal_data.json entry
            if doc.metadata.get("source_entry"):
                fiscal_filter = {
                    "$and": [
                        {"id": doc.metadata["source_entry"]},
                        {"language": language}
                    ]
                }
                fiscal_context = vectorstore.similarity_search(
                    query="", k=1, filter=fiscal_filter
                )
                if fiscal_context:
                    fiscal_content = fiscal_context[0].page_content
                    filtered.append(fiscal_content)
                    logger.info(f"Matched fiscal_data.json entry for source_entry: {doc.metadata['source_entry']}")
    
    return "\n".join(filtered) if filtered else context[0].page_content if context else "No relevant context found."
...
async def chat(input: ChatInput):
    ...
    # Use strict language and intent filter, including source_entry matching
    where_filter = {
        "$and": [
            {"language": language},
            {"$or": [
                {"intent": intent},
                {"intent": "none"},
                {"category": {"$in": ["tax_rate", "deadline", "penalty", "social_security"]}}  # Include fiscal categories
            ]}
        ]
    }
